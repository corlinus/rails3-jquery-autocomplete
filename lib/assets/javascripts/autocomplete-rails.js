...
function(){var c={},e=a(b).attr("data-autocomplete-send");a(a(e).serializeArray()).each(function(a,b){c[b.name]=b.value});c.term=g.term.split(b.delimiter).pop().replace(/^\s+/,"");return c}(),function(){a(arguments[0]).each(function(c,e){var d={};d[e.id]=e;a(b).data(d)});d.apply(null,arguments)})},search:function(){if(2>this.value.split(b.delimiter).pop().replace(/^\s+/,"").length)return!1},focus:function(){return!1},select:function(g,d){var c=this.value.split(b.delimiter);c.pop();c.push(d.item.value);
if(null!=b.delimiter)c.push(""),this.value=c.join(b.delimiter);else if(this.value=c.join(""),a(this).attr("data-id-element")&&a(a(this).attr("data-id-element")).val(d.item.id),a(this).attr("data-update-elements")){var c=a(this).data(d.item.id.toString()),e=a.parseJSON(a(this).attr("data-update-elements")),f;for(f in e)a(e[f]).val(c[f])}var h=this.value;a(this).bind("keyup.clearId",function(){a(this).val().trim()!=h.trim()&&(a(a(this).attr("data-id-element")).val(""),a(this).unbind("keyup.clearId"))});
