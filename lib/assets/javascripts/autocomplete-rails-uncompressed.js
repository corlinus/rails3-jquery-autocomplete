    ...
    init: function(e) {
      ...
      jQuery(e).autocomplete({
        source: function( request, response ) {
          jQuery.getJSON( jQuery(e).attr('data-autocomplete'), function() {
              var data = {}, selector = jQuery(e).attr('data-autocomplete-send');
              jQuery(jQuery(selector).serializeArray()).each(
                function(i,el) { data[el.name] = el.value}
              );
        ...
        search: function() {
          ...
          if ( term.length < (jQuery(e).attr('data-autocomplete-min-length') || 2) ) {
            return false;
          }
        },
        focus: function() {
